...
(function initializeDaliaChatClient() {
    ...
    function parseEvent(rawEvent, callbacks, state) {
        ...
        if (event.error) {
            const error = errorWithCode(String(event.error), event.code || 'DALIA_STREAM_ERROR');
            error.retryAfterMs = Number(event.retry_after_ms) || null;
            throw error;
        }
        if (event.handoff) {
        ...
        if (event.status?.type) callbacks.onStatus?.(event.status);
    ...
    async function streamChat({ messages, sessionId, onDelta, onHandoff, onStatus, signal }) {
        ...
        if (!(health.ai_online ?? health.lmstudio_online) && health.ai_retry_allowed !== true) {
            throw errorWithCode('El proveedor de inteligencia de Dalia no está disponible.', 'AI_PROVIDER_OFFLINE');
        }
        ...
        const callbacks = { onDelta, onHandoff, onStatus };
        let buffer = '';
